...
describe('Services', () => {
  ...
  it('It Should Test getCustomers of service file with data returned', () => {
    let mockedres= Rx.of([   
    ...
    let getCountriesPhones = spyOn(service,"getCustomers").and.callFake(() => {
      return mockedres;
    });
    expect(service.getCustomers('all',  'all')).toEqual(mockedres);
  });

  it('It Should Test getCustomers of service file with no data returned', () => {
    let mockedres= Rx.of([ ]);   
    let getCountriesPhones = spyOn(service,"getCustomers").and.callFake(() => {
      return mockedres;
    });
